...
import { Home, Activity, FileText, Settings, User, LogOut } from "lucide-react";
    ...
    <div className="min-h-screen flex bg-[#F8FAFC]">
      ...
      <aside className="hidden md:flex flex-col w-72 bg-white/80 backdrop-blur-xl border-r border-slate-200 p-6 fixed h-full z-20">
        ...
        <nav className="space-y-1 flex-1">
          <NavLink to="/dashboard" icon={<Home />} label="Overview" active={location.pathname === "/dashboard"} />
          <NavLink to="/dashboard/patients" icon={<User />} label="Patients" active={location.pathname === "/dashboard/patients"} />
          <NavLink to="/dashboard/reports" icon={<FileText />} label="Reports" active={location.pathname === "/dashboard/reports"} />
          <div className="my-4 h-px bg-slate-100 mx-2" />
          <Link to="/test" className="flex items-center gap-3 px-3 py-3 rounded-xl bg-slate-900 text-white shadow-lg shadow-slate-900/10 hover:scale-[1.02] transition-transform">
